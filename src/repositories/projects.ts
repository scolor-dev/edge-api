...
type ProjectMutableFields = Omit<Project, 'id' | 'created_at' | 'updated_at' | 'deleted_at' | 'delete_after'>
...
export const projectRepository = {
   ...
   */
  async getById(db: D1Database, id: string, includeDeleted = false): Promise<ProjectWithTags | null> {
   ...
   * - includeDeleted: trueの場合は論理削除済みも取得（admin用）
   */
  async getBySlug(db: D1Database, slug: string, includeDeleted = false): Promise<ProjectWithTags | null> {
    const result = await db
      .prepare(`
        SELECT
          p.*,
   ...
   * - D1: projects にINSERT
   */
  async create(db: D1Database, data: Omit<Project, 'created_at' | 'updated_at' | 'deleted_at' | 'delete_after'>): Promise<void> {
    const now = new Date().toISOString()
    await db
      .prepare(`
        INSERT INTO projects (id, has_index, title, slug, thumbnail, description, links, keywords, date, status, deleted_at, delete_after, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, NULL, ?, ?)
      `)
      .bind(
        data.id, data.has_index, data.title, data.slug,
        data.thumbnail, data.description, data.links,
  ...
  async replace(db: D1Database, id: string, data: ProjectMutableFields): Promise<void> {
    ...
    await db
      .prepare(`
        UPDATE projects
        SET has_index = ?, title = ?, slug = ?, thumbnail = ?, description = ?, links = ?,
            keywords = ?, date = ?, status = ?, updated_at = ?
        WHERE id = ?
          AND deleted_at IS NULL
      `)
      .bind(
        data.has_index, data.title, data.slug, data.thumbnail,
        data.description, data.links, data.keywords,
        data.date, data.status, now, id
      )
      .run()
  },
