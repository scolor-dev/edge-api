...
export const tagCategoryRepository = {
	...
	async getAll(db: D1Database): Promise<TagCategory[]> {
		const { results } = await db.prepare("SELECT * FROM tag_categories ORDER BY name").all()
	...
	async create(
		db: D1Database,
		data: Omit<TagCategory, "created_at" | "updated_at">,
	): Promise<void> {
		const now = new Date().toISOString()
		await db
			.prepare(
				"INSERT INTO tag_categories (id, name, slug, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
			)
	...
	async update(
		db: D1Database,
		id: string,
		data: Pick<TagCategory, "name" | "slug">,
	): Promise<void> {
		const now = new Date().toISOString()
		await db
...
export const tagRepository = {
	...
	async create(db: D1Database, data: Omit<Tag, "created_at" | "updated_at">): Promise<void> {
		...
		await db
			.prepare(
				"INSERT INTO tags (id, category_id, name, slug, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)",
			)
